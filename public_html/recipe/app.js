...
function loadFavs() {
    try {
        return new Set(JSON.parse(localStorage.getItem(LS_KEY_FAVS) || '[]'));
    } catch {
        return new Set();
    }
}
...
function toggleFav(id) {
    if (isFav(id)) FAVS.delete(id); else FAVS.add(id);
    saveFavs(FAVS);
}
...
function renderDetail(id) {
    ...
    const dh = $('#detail-heading');
    if (dh) dh.textContent = r.title;
    const dt = $('#d-time');
    if (dt) dt.textContent = r.time || '';
    const tg = $('#d-tags');
    if (tg) tg.textContent = (r.tags || []).map(t => '#' + t).join(' ');
    const ing = $('#d-ingredients');
    if (ing) ing.innerHTML = (r.ingredients || []).map(i => '<li>' + esc(i) + '</li>').join('');
    const st = $('#d-steps');
    if (st) st.innerHTML = (r.steps || []).map(s => '<li>' + esc(s) + '</li>').join('');
...
async function init() {
    ...
    if ($('#backFromTags')) $('#backFromTags').addEventListener('click', (e) => {
        if (e.button !== 0) return;
        e.preventDefault();
        navigate('list', {});
    });
    ...
    // タグページへ（ヘッダ）
    if ($('#navTags')) $('#navTags').addEventListener('click', (e) => {
        if (e.button !== 0) return;
        e.preventDefault();
        navigate('tags', {});
    });

// Service Worker（?sw=1 の時だけ）
    if ('serviceWorker' in navigator && new URLSearchParams(location.search).get('sw') === '1') {
        try {
